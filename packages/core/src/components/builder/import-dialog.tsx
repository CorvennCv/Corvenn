      ...
      if (isJSON) {
        ...
        if (!response.success) {
          if (response.error?.includes("503") || response.error?.includes("upstream")) {
            throw new Error("The AI service is currently overloaded or unavailable. Please try again in a few minutes or upload a JSON/Corvenn file instead.")
          }
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload a `.json`, `.corvenn`, or `.pdf` file. We leverage AI directly to ingest standard PDFs seamlessly!
          </DialogDescription>
        </DialogHeader>
        ...
        >
          {isProcessing ? (
            ...
            <>
              ...
              <input
                type="file"
                accept=".json,application/json,.corvenn,.pdf,application/pdf"
                className="hidden"
                ref={fileInputRef}
